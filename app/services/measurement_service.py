...
_BKT_P_MAX = 0.999      # p_mastery 上限 < 1.0：防 p=1.0 吸收态——p=1.0 时 (1-p)=0 使答错的
                        # 贝叶斯后验恒为 1.0，该 KP 永远免疫"答错降掌握度"，违反 BKT 单调性
                        # （审计 L2-001：27 连对即收敛到 1.0 触发）。钳到 0.999 保证 (1-p)>0。
...
def bkt_update(
    ...
    *, p_transit: float = _BKT_P_TRANSIT, p_slip: float = _BKT_P_SLIP,
    ...
    p = min(_BKT_P_MAX, max(0.0, p))  # 钳上限 <1.0，保证 (1-p)>0（防吸收态，含历史已存 1.0 数据）
    if correct:
        num = p * (1 - p_slip)
    ...
    return min(_BKT_P_MAX, max(0.0, p_post))
