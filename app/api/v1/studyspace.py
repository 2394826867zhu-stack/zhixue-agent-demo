...
from app.schemas.studyspace import (
    StartSessionRequest, UpdateSessionRequest, LessonProgress,
    StudySpaceSessionOut, CompleteSessionResponse,
)
...
@router.post("/sessions", summary="开始课时学习会话", response_model=Envelope[StudySpaceSessionOut])
...
@router.get("/sessions", summary="用户历史会话列表", response_model=Envelope[list[StudySpaceSessionOut]])
...
@router.get("/sessions/{session_id}", summary="获取会话状态", response_model=Envelope[StudySpaceSessionOut])
...
@router.patch("/sessions/{session_id}", summary="更新会话进度", response_model=Envelope[StudySpaceSessionOut])
...
@router.post("/sessions/{session_id}/complete", summary="完成课时（触发知识点提取+闪卡生成）", response_model=Envelope[CompleteSessionResponse])
...
@router.get("/sessions/{session_id}/timeline", summary="StudySpace 垂直时间线（沉淀学习记录）", response_model=Envelope[list[TimelineNodeOut]])
...
@router.post("/sessions/{session_id}/timeline", summary="用户主动追加内容 / 复盘节点", response_model=Envelope[TimelineNodeOut])
...
# TODO(SDD): 精化 spot_quiz_service.generate_for_kp 的返回 schema（当前 Envelope[dict]）
@router.post("/sessions/{session_id}/spot-quiz", summary="为某 KP 自动出随堂测验（v0.33 P0-2）", response_model=Envelope[dict])
...
@router.get("/timeline-nodes/{node_id}", summary="时间线节点详情（v0.32）", response_model=Envelope[TimelineNodeOut])
...
@router.patch("/timeline-nodes/{node_id}", summary="编辑时间线节点（仅可编辑节点）", response_model=Envelope[TimelineNodeOut])
