...
@router.get("/status", summary="获取当前引导步骤（自动创建会话）", response_model=None)
...
@router.post("/chat", summary="发送引导对话消息，推进状态机", response_model=None)
async def chat(
...
@router.post("/restart", summary="重置引导对话（重新填写）", response_model=None)
async def restart(
    user: User = Depends(get_current_user),
    db: AsyncSession = Depends(get_db),
):
