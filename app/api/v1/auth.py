...
@router.post("/register", summary="注册", response_model=Envelope[TokenResponse])
...
@router.post("/login", summary="登录", response_model=Envelope[TokenResponse])
async def login(body: LoginRequest, db: AsyncSession = Depends(get_db)):
    tokens = await auth_service.login(db, body)
    return ok(TokenResponse(**tokens))


@router.post("/refresh", summary="刷新 Token", response_model=Envelope[TokenResponse])
async def refresh(body: RefreshRequest):
    tokens = await auth_service.refresh_token(body.refresh_token)
    return ok(TokenResponse(**tokens))


@router.post("/logout", summary="登出", response_model=Envelope[None])
async def logout(body: RefreshRequest, _: User = Depends(get_current_user)):
    await auth_service.logout(body.refresh_token)
    return ok(None)


@router.get("/me", summary="获取当前用户信息", response_model=Envelope[UserProfile])
async def get_me(user: User = Depends(get_current_user)):
    return ok(UserProfile.model_validate(user))


@router.patch("/me", summary="更新用户资料", response_model=Envelope[UserProfile])
async def update_me(
