...
## ✅ 修复完成（2026-06-01，commit 86f871b + 测试修正 9c4f9e4，全套 139 pass）
...
全套从 134 → **139 passed**（+4 e2e +1 钳制；删的重复测试原与另一同名函数互相遮蔽不影响计数）。main HEAD=9c4f9e4。

**诚实记录（过程教训）**：修复提交 86f871b 一度包含 3 个 FAILING 的 e2e 测试就被提交了——根因是我写的 fake `_grade_answer` 返回元组顺序错（真实是 `(score, feedback, is_wrong, error_reason)`）+ `_post_review_side_effects` 是模块级函数却按类方法 patch。源码三钩子本身正确，是测试代码写错。已在 9c4f9e4 修正，全套真绿。教训叠加：**提交前必须亲眼确认全套绿数字，不能凭推断写"139 pass"。**
